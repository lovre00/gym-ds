...
# Function to predict gym attendance based on forecast data
...
def get_weather_forecast(latitude, longitude):
    ...
    # Open-Meteo endpoint for hourly weather forecast
    ...
    # Extract the relevant forecast data for the current hour
...
# Example usage
latitude = 43.5089  # Example: New York City latitude
longitude = 16.4392  # Example: New York City longitude
hour = 19  # Example: 3 PM
...
def predict_gym_attendance(forecast_data):
    ...
    # Create a DataFrame with placeholder values for all required features
    forecast_df = pd.DataFrame(columns=required_features)

    # Update the DataFrame with the provided forecast data
    for feature in required_features:
        if feature in forecast_data:
            forecast_df.at[0, feature] = forecast_data[feature]
        else:
            forecast_df.at[0, feature] = 0  # or a suitable default value
...
# Print the predicted attendance for each gym
# print("Predicted Attendance - GymA: {}, GymB: {}, GymC: {}".format(*attendance_prediction[0]))
